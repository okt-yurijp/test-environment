...
def gtest_get_node(line):
    ...
    if result != None:
        return "test", result.group(3), f'{result.group(1)}.{result.group(2)}'
...
def gtest_list_tests_gen(gtest_bin):
    ...
    """
    parent = None
    output = subprocess.check_output([gtest_bin, "--list-tests"])
    ...
    for line in filter(bool, list_tests_str.split("\n")):
        ...
        if not node:
            ...
            continue
        node_type, node_name, parent_node_name = node
...
def generate_pkg(args, gtest_tests):
    ...
    gtest_bin2 = 'rdma_core' #gtest_bin.split('.')[0]
    basedir = os.path.join(args.pkg_dir, gtest_bin2)
    ...
    for parent, tests in gtest_tests.items():
        ...
        pkg.add_session_arg(GTestDef.env_arg_name)
